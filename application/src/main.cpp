...
void per_quad_faces_normals(Eigen::MatrixXd& V, Eigen::MatrixXi& F, Eigen::MatrixXd& N)
{
  ...
  // Loop over the faces
#pragma omp parallel for if (facesCount > 100)
  for (int i = 0; i < facesCount; ++i)
  {
    Eigen::RowVector3d p0 = V.row(F(i, 0)), p1 = V.row(F(i, 1)),
      p2 = V.row(F(i, 2)), p3 = V.row(F(i, 3));
    auto n1 = (p2 - p1).cross(p0 - p1);
    auto n2 = (p0 - p3).cross(p2 - p3);
    n1.normalize();
    n2.normalize();
    Eigen::RowVector3d newNormal = n1 + n2 / 2.;
    newNormal.normalize();
    N.row(i) = newNormal;
  }
}
...
void new_vertex_pos(Eigen::MatrixXd& V, Eigen::MatrixXd& normals, std::vector<bool>& tombStonesV,
  ...
  std::priority_queue<Segment, std::vector<Segment>, CompareTwoSegments>& operations,
  std::priority_queue<Segment, std::vector<Segment>, CompareTwoSegments>& delOperations, Eigen::VectorXd curvatures)
{
  if (!tombStonesV[oldVert]) { return; }

  if (borderVerts.count(oldVert))
  {
    return; // Vertex belongs to the mesh border
  }

  if (curvatures.row(oldVert)[0] > 1.) { return; }
  
  Eigen::RowVector3d centroid = { 0., 0., 0. };
  Eigen::RowVector3d rawOldVert = V.row(oldVert);
  int size = 0;
  Eigen::RowVector4i f;
  ...
  for (int face : adt[oldVert])
  {
    if (tombStonesF[face])
    {
      ...
      for (int i = 0; i < 4; ++i)
      {
        if (f[i] == oldVert)
        {
          centroid += (V.row(f[0]) + V.row(f[1]) + V.row(f[2]) + V.row(f[3])) / 4.0;
  ...
  std::set<int> vertsOnRing)
{
  ...
  for (int vRing : vertsOnRing)
  {
    normal += normals.row(vRing);
  }
    ...
    if (tombStonesF[face])
    {
      ...
      for (int i = 0; i < 4; ++i)
      {
        if (f[i] == vM)
        {
          ...
          insertSegment(vM, nextVert, false, V, operations);
          insertSegment(vM, oppositeVert, true, V, operations);
  ...
  Eigen::VectorXd curvatures)
{
  ...
  Eigen::RowVector3d origD = V.row(vertexToBeRemoved);
  ...
  if (curvatures.row(vertexToBeRemoved)[0] > 1.)
  {
      ...
      if (tombStonesF[face])
      {
        ...
        for (int i = 0; i < 4; ++i)
        {
          if (f[i] == endVert1)
          {
            ...
            removeSegment(endVert1, nextVert, false, V, delOperations);
            removeSegment(endVert1, oppositeVert, true, V, delOperations);
    ...
    V.row(endVert1) = 0.1 * V.row(endVert1) + 0.9 * V.row(vertexToBeRemoved);
    curvatures.row(endVert1) = curvatures.row(vertexToBeRemoved);
      ...
      if (tombStonesF[face])
      {
        ...
        for (int i = 0; i < 4; ++i)
        {
          if (f[i] == endVert1)
          {
            ...
            insertSegment(endVert1, nextVert, false, V, operations);
            insertSegment(endVert1, oppositeVert, true, V, operations);
      ...
      if (tombStonesF[face])
      {
        ...
        for (int i = 0; i < 4; ++i)
        {
          if (f[i] == endVert2)
          {
            ...
            removeSegment(endVert2, nextVert, false, V, delOperations);
            removeSegment(endVert2, oppositeVert, true, V, delOperations);
    ...
    V.row(endVert2) = 0.1 * V.row(endVert2) + 0.9 * V.row(vertexToBeRemoved);
    curvatures.row(endVert2) = curvatures.row(vertexToBeRemoved);
      ...
      if (tombStonesF[face])
      {
        ...
        for (int i = 0; i < 4; ++i)
        {
          if (f[i] == endVert2)
          {
            ...
            insertSegment(endVert2, nextVert, false, V, operations);
            insertSegment(endVert2, oppositeVert, true, V, operations);
  ...
  if (v1Valence == 2) // Doublet found
  {
    remove_doublet(endVert1, V, normals, tombStonesV, borderVerts, F, tombStonesF, 
      aliveFaces, adt, operations, delOperations, curvatures);
  }
...
bool try_vertex_rotation(int rotationVert, Eigen::MatrixXd& V, Eigen::MatrixXd& normals,
  ...
  std::priority_queue<Segment, std::vector<Segment>, CompareTwoSegments>& operations,
  std::priority_queue<Segment, std::vector<Segment>, CompareTwoSegments>& delOperations,
  Eigen::VectorXd curvatures, int vertForNextCollapse = -1)
{
    ...
    for (int vertDoublet : vertsForDoublets)
    {
      ...
      if (valence == 2) // Doublet found
      {
        ...
        remove_doublet(vertDoublet, V, normals, tombStonesV, borderVerts, F, 
          tombStonesF, aliveFaces, adt, operations, delOperations, curvatures);
      }
    }
...
bool try_edge_rotation(int vert1, int vert2, Eigen::MatrixXd& V, Eigen::MatrixXd& normals,
  ...
  std::priority_queue<Segment, std::vector<Segment>, CompareTwoSegments>& operations,
  std::priority_queue<Segment, std::vector<Segment>, CompareTwoSegments>& delOperations,
  Eigen::VectorXd curvatures, bool forceRotation = false)
{
  ...
  for (Segment& edge : involvedEdges)
  {
    if (tombStonesV[edge.v2] && tombStonesV[edge.v1])
    {
      if (!try_edge_rotation(edge.v1, edge.v2, V, normals, tombStonesV, borderVerts, 
        F, tombStonesF, aliveFaces, adt, operations, delOperations, curvatures))
      {
        return false;
      }
    }
...
bool diagonal_collapse(const Segment& diag, Eigen::MatrixXd& V, Eigen::MatrixXd& normals,
  ...
  Eigen::VectorXd curvatures)
{
  int vertexToBeMaintained, vertexToBeRemoved;

  if (curvatures.row(diag.v1)[0] > curvatures.row(diag.v2)[0])
  {
    vertexToBeMaintained = diag.v1;
    vertexToBeRemoved = diag.v2;
  }
  else
  {
    vertexToBeMaintained = diag.v2;
    vertexToBeRemoved = diag.v1;
  }
  ...
  bool isHalfOnBorder = isOnBorder ? false :
  ...
  Eigen::RowVector3d origM = rawVertToBeMaintained, origR = rawVertToBeRemoved;
      ...
      for (int v2 : verts2)
      {
        if (v1 == v2)
        {
          ...
          try_edge_rotation(e.v1, e.v2, V, normals, tombStonesV, borderVerts, F, tombStonesF,
            aliveFaces, adt, operations, delOperations, curvatures, true);
  ...
  if (isOnBorder)
  {
    ...
    {
      removeSegment(vertexToBeMaintained, vertexToBeRemoved, true, V, delOperations);
      ...
      if (tombStonesF[face])
      {
        ...
        for (int i = 0; i < 4; ++i)
        {
          if (f[i] == vertexToBeMaintained)
          {
            ...
            vertsOnRing.emplace(nextVert);
            vertsOnRing.emplace(oppositeVert);
  ...
  { 
    if (curvatures.row(vertexToBeMaintained)[0] < 1.)
    {
  ...
  Eigen::VectorXd curvatures)
{
  ...
  if (!try_vertex_rotation(vert1, V, normals, tombStonesV, borderVerts, F, tombStonesF,
    aliveFaces, adt, operations, delOperations, curvatures, vert2)) // Force rotation
  {
  ...
  Eigen::VectorXd curvatures)
{
  ...
  if (tombStonesV[survivedVertex])
  {
    ...
    optimize_quad_mesh(edgesFromVertex, survivedVertex, V, normals, tombStonesV,
      borderVerts, F, tombStonesF, aliveFaces, adt, operations, delOperations, curvatures);
  }
...
bool start_simplification(Eigen::MatrixXd& V, Eigen::MatrixXi& F, int finalNumberOfFaces)
{
  ...
  Eigen::VectorXd curvatures;
  igl::gaussian_curvature(V, f, curvatures);

  // Try to coarsen the quad mesh
  std::cout << "Initial number of quad faces: " << F.rows() << "\n";
  while (aliveFaces > finalNumberOfFaces)
  {
    ...
    printf("\rCurrent quad faces of the mesh: %d ", aliveFaces);
    fflush(stdout);
  }

  // Output preparation
  Eigen::MatrixXi newF(aliveFaces, 4);
  int counter = 0;
  for (int i = 0; i < tombStonesF.size(); ++i)
  {
    if (tombStonesF[i] && counter < aliveFaces)
    {
      newF.row(counter++) << F.row(i);
    }
  }
  F.conservativeResizeLike(newF);
  F = newF;
...
{
  const std::string MESH_DIR_PATH = ""; // Write the directory path containing the quad mesh
  const std::string MESH_NAME = ""; // Write the quad mesh name without extension (e.g.: David)
  const int NUM_FACES = 1000; // Write the number of final quad-faces (e.g.: 1000)

  Eigen::MatrixXd V;
  Eigen::MatrixXi F;
  Eigen::MatrixXd N;

  // Load the input quad mesh
  igl::readOBJ(MESH_DIR_PATH + MESH_NAME + ".obj", V, F);

  std::cout << "Quad mesh coarsening in progress...\n\n";

  if (start_simplification(V, F, NUM_FACES))
  {
    std::cout << "Quad mesh drawing in progress...\n\n";

    // Create the final quad mesh
    std::time_t time = std::time(nullptr);
    std::localtime(&time);
    igl::writeOBJ(MESH_DIR_PATH + MESH_NAME + std::to_string(time) + ".obj", V, F);

    draw_quad_mesh(V, F);
  }
  else
  {
    std::cout << "\n\n" << "Sorry, it was not possible to simplify the mesh up to " <<
      "the indicated number of faces. This could be due to a bad conformation of the original " << 
      "mesh. But if not, try to use a larger number of faces please.\n\n";
  }
}
